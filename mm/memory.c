...
{
	struct page *page = lookup_swap_cache(entry);                   // nxj 换入内存的时候 先看下 cache 里面是否有
	pte_t pte;

	if (!page) {
		lock_kernel();
		swapin_readahead(entry);                                    // nxj 预读 内存页面 集群
		page = read_swap_cache(entry);                              // nxj 将 cache 里面的页面读入 vma 中
		unlock_kernel();
		if (!page)
			return -1;
